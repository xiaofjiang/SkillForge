...
- **Guardrails are not a procedure. Ship both.** A skill made only of gates, refusals, anti-pattern tables and error classes tells a session when it has gone WRONG but never what to DO, in what order, checking what, before moving on. Every session then improvises the path between the gates, and each improvises differently because it cannot see what the last one chose, so the errors rhyme without repeating. Any skill with more than a couple of gates needs numbered, mostly-required steps stated concretely enough that two different sessions produce the same artifacts in the same places. Two constraints that come with it: branch by SHAPE rather than writing one long file, because a runbook long enough to cover everything becomes a document nobody reads and is then decoration that also lies; and every human-review step carries a note saying what would have to be true to retire it. One step earns its place by default: before editing any file, state what is BOUND to it, because on 2026-08-25 a correct five-string fix silently invalidated three receipts when nothing in the process asked what points at this. Diagnosis is Xiao's, 2026-08-25, carried here from bead claude-code-9m6f (catalog runbook) when that bead was closed under ruling D5-B on 2026-09-07: the catalog pipeline it was written for is retired, the observation about skills is not.
...
Every NEW skill is substantive per the `verify-harness` size gate and takes its skill-authoring lane; the subagent test below is that lane's minimum stage. For a skill whose failure would cost real data or trust (data pipelines, live-write wrappers, verification/gate skills), run the lane's full sequence (adversarial panel to unanimous, fresh-subagent trigger test with counts, cross-model audit with pre-mortem). vollis-catalog and verify-harness itself were both validated that way.
